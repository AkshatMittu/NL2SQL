import torch
import random
from utils import metadata_toon, new_metadata, prompts, use_mistral, get_args, kpi_info_toon, run_query, update_db
...
def nl2sql(nl_query):
  ...
  relevance_op = eval(use_mistral(rel_prompt))
  # relevance_op = eval(relevance_op.split("```")[1][4:]) #JSON
  ...
  if relevance_op['relevance'] == 'No':
    ...
    return relevance_op['explanation'], []
    ...
    print("Knowledge OP")
    print(knowledge_op)
    knowledge_op = eval(knowledge_op) #JSON
    ...
    sql_query = eval(sql_op)['sql_query']
    ...
    for i in range(n_iters):
      ...
      try:
        ...
        syntax_op = use_mistral(syntax_prompt, op_type="text")
        print()
        print("Syntax OP:")
        print(syntax_op)
        ...
        print("SQL OP:")
        print(sql_op)
        sql_query = eval(sql_op)['corrected_sql_query']
        args['sql_query'] = sql_query
        continue
      ...
      reversed = use_mistral(reverse_prompt, op_type="text")
      print()
      print("Reverse OP:")
      print(reversed)
